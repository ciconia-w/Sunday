...
async function withDiscordCollector(run) {
    const calls = [];
    ...
    try {
        return await run({
            webhookUrl: `http://127.0.0.1:${port}/reply-discord`,
            calls,
            waitForCallCount,
        });
    } finally {
...
const now = Date.now();
const source = "im-discord-demo";
const channelId = `discord-room-${now}`;
const threadId = `discord-thread-${now}`;
const firstExternalMessageId = `discord-first-${now}`;
const secondExternalMessageId = `discord-second-${now}`;
const runtimeDir = await mkdtemp(join(tmpdir(), "sunday-ingress-discord-verify-"));
...
try {
    ...
    await withDiscordCollector(async ({ webhookUrl, calls, waitForCallCount }) => {
            ...
            async ({ sidecarPort }) => {
                ...
                try {
                    firstIngressResult = await post(baseUrl, "/ingress/message", {
                        ...
                        text: "Reply with exactly: ingress-discord-first-ok",
                        externalMessageId: firstExternalMessageId,
                        replyTransport: "discord-incoming-webhook",
                    ...
                    if (!firstCycle.ok) {
                        throw new Error(`discord ingress verifier first cycle did not finish: ${firstCycle.reason}`);
                    }
                    ...
                    secondIngressResult = await post(baseUrl, "/ingress/message", {
                        ...
                        userId: "demo-user",
                        assistantId: "uos-ai-generic",
                        text: "Reply with exactly: ingress-discord-followup-ok",
                    ...
                    if (!secondCycle.ok) {
                        throw new Error(`discord ingress verifier follow-up cycle did not finish: ${secondCycle.reason}`);
                    }
    ...
    const checks = {
        ...
        firstReplyUsesDiscordBody: firstCall?.body?.content === "ingress-discord-first-ok",
        secondReplyUsesStoredRoute: secondCall?.body?.content === "ingress-discord-followup-ok",
        providerDoesNotUseSlackShape: !("text" in (firstCall?.body ?? {})),
        routeSummaryNormalizesDiscordTransport: storedRoute?.replyTarget?.transport === "discord-webhook",
        persistedRouteNormalizesDiscordTransport: persistedRoute?.replyTarget?.transport === "discord-webhook",
        persistedRouteStoresWebhookUrl: typeof persistedRoute?.replyTarget?.url === "string"
            && persistedRoute.replyTarget.url.includes("/reply-discord"),
    };

    const verdict = Object.values(checks).every(Boolean)
        ? "ingress-discord-api-confirmed"
        : "ingress-discord-api-incomplete";
    ...
    process.exit(verdict === "ingress-discord-api-confirmed" ? 0 : 1);
} finally {
