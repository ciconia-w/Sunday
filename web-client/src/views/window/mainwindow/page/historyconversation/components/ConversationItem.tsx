...
function highlightText(text: string, keyword: string): (string | VNode)[] {
    ...
    while (index !== -1) {
        ...
        parts.push(
            <span class="conversation-item__highlight">{text.slice(index, index + keyword.length)}</span>,
        );
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const modelLabel = computed(() => {
            const modelName = props.conversation.model_name?.trim();
            if (modelName) {
                return modelName;
            }

            const modelId = props.conversation.model?.trim();
            if (!modelId) {
                return "";
            }
        ...
        const runtimeModeLabel = computed(() => {
            const runtimeMode = props.conversation.runtime_mode?.trim();
            if (!runtimeMode) {
                return "";
            }
            ...
            if (runtimeMode === "local-mock" || runtimeMode === "mock") {
                return "MOCK";
            }
        ...
        const safeConversationTitle = computed(() => {
            return String(props.conversation.title ?? "").trim() || backendStore.translate("新对话");
        });
                ...
                <div class="conversation-item__wrapper" onClick={this.handleWrapperClick}>
                    ...
                    >
                        <div class="conversation-item__header">
                            ...
                            <Tooltip content={this.safeConversationTitle} showAfter={1000} placement="top-start">
                                <div class="conversation-item__title">
                                    {this.highlightText(this.safeConversationTitle, this.searchKeyword)}
                                </div>
                            </Tooltip>
                            {this.providerLabel && (
                                <span class="conversation-item__provider-badge">{this.providerLabel}</span>
                            )}
                            {this.modelLabel && (
                                <span class="conversation-item__model-badge">{this.modelLabel}</span>
                            )}
                            {this.runtimeModeLabel && (
                                <span class="conversation-item__runtime-badge">{this.runtimeModeLabel}</span>
                            )}
                        ...
                        {this.conversation.introduction &&
                            ...
                            ) && <div class="conversation-item__description">{this.highlightText(this.conversation.introduction, this.searchKeyword)}</div>}
                    </div>
                </div>
                <Menu
