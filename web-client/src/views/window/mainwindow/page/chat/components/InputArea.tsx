...
export default defineComponent({
    ...
    emits: {
        ...
        // 发送按钮点击事件
        send: null,
        applySuggestedPrompt: (payload: { prompt: string; send: boolean }) => {
            return typeof payload?.prompt === "string" && typeof payload?.send === "boolean";
        },
        // 深度思考按钮点击事件
        deepThink: null,
        // 联网搜索按钮点击事件
    ...
    setup(props, { emit }) {
        ...
        const fileStatusSummary = computed(() => {
            const files = uploadFilesStore.uploadedFiles;
            const total = files.length;
            const parsingCount = files.filter((file) => file.parseStatus === "pending" || file.parseStatus === "parsing").length;
            const errorCount = files.filter((file) => file.parseStatus === "error").length;
            const readyCount = files.filter((file) => !file.parseStatus || file.parseStatus === "completed").length;
        ...
        const fileGuidanceTitle = computed(() => {
            if (fileStatusSummary.value.parsingCount > 0) {
                return backend.translate("Files are still being prepared");
            }
            if (fileStatusSummary.value.errorCount > 0) {
                return backend.translate("Some files could not be parsed");
            }
            if (fileStatusSummary.value.total > 0) {
                return backend.translate("Files are attached and ready");
            }
            return "";
        });

        const fileGuidanceDescription = computed(() => {
            const { total, parsingCount, errorCount, readyCount } = fileStatusSummary.value;

            if (parsingCount > 0) {
                return backend.translate(
                    `${parsingCount} file(s) are still parsing. You can wait for richer context or start asking about the files now.`,
                );
            }

            if (errorCount > 0) {
                return backend.translate(
                    `${readyCount} file(s) are ready. ${errorCount} file(s) failed to parse, but you can still remove them or continue with the rest.`,
                );
            }

            if (total > 0) {
                return backend.translate(
                    "你可以让 Sunday 总结这些文件、比较差异，或者直接说明要提取和修改什么。",
                );
            }
        ...
        const filePromptSuggestions = computed(() => {
            if (fileStatusSummary.value.total === 0) {
                return [];
            }

            return [
                backend.translate("Summarize the uploaded files and list the key points."),
                backend.translate("Compare the uploaded files and highlight the important differences."),
                backend.translate("Read the uploaded files and tell me what needs follow-up first."),
            ];
        });

        const failedUploadedFiles = computed(() => {
            return uploadFilesStore.uploadedFiles.filter((file) => file.parseStatus === "error");
        });
        const isClearingAllFiles = ref(false);
        const isRetryingFailedFiles = ref(false);
        const isRemovingFailedFiles = ref(false);
        const failedFilesCleanupFeedback = ref("");
        let failedFilesCleanupFeedbackTimer: number | null = null;
        const failedFilesRetryFeedback = ref("");
        let failedFilesRetryFeedbackTimer: number | null = null;
        const attachmentsBulkFeedback = ref("");
        let attachmentsBulkFeedbackTimer: number | null = null;
        ...
        const applySuggestedPrompt = (prompt: string, send = false) => {
            if (props.disabled || !prompt) {
                return;
            }

            textareaRef.value?.focus();
            textareaRef.value?.insertTextAtCursor(prompt);
            emit("updateValue", prompt);
            emit("input", prompt);

            if (send) {
                nextTick(() => {
                    emit("applySuggestedPrompt", { prompt, send: true });
                });
            }
        };

        const handleRemoveFailedFiles = async () => {
            if (
                failedUploadedFiles.value.length === 0 ||
                isRemovingFailedFiles.value ||
                isRetryingFailedFiles.value ||
                isClearingAllFiles.value
            ) {
                return;
            }

            isRemovingFailedFiles.value = true;
            attachmentsBulkFeedback.value = "";
            failedFilesRetryFeedback.value = "";
            const filesToRemove = failedUploadedFiles.value.map((file) => file.filePath);

            try {
                const removalResults = await Promise.allSettled(
                    filesToRemove.map((filePath) => uploadFilesStore.removeFile(filePath)),
                );
                const removedCount = removalResults.filter((result) => result.status === "fulfilled").length;

                removalResults.forEach((result) => {
                    if (result.status === "rejected") {
                        console.error("Failed to remove errored file:", result.reason);
                    }
                });
                ...
                failedFilesCleanupFeedback.value = backend.translate(`${removedCount} failed file(s) removed`);
                if (failedFilesCleanupFeedbackTimer !== null) {
                    window.clearTimeout(failedFilesCleanupFeedbackTimer);
                }
                failedFilesCleanupFeedbackTimer = window.setTimeout(() => {
                    failedFilesCleanupFeedbackTimer = null;
                    failedFilesCleanupFeedback.value = "";
                }, 2600);
            } finally {
                isRemovingFailedFiles.value = false;
            }
        };
        ...
        const handleRetryFailedFiles = async () => {
            ...
            isRetryingFailedFiles.value = true;
            attachmentsBulkFeedback.value = "";
            failedFilesCleanupFeedback.value = "";
            const filesToRetry = failedUploadedFiles.value.map((file) => ({
                id: file.id,
                filePath: file.filePath,
            }));
            ...
            try {
                const retryResults = await Promise.allSettled(
                    filesToRetry.map(({ id, filePath }) => uploadFilesStore.requestParse(id, filePath)),
                );
                const retriedCount = retryResults.filter((result) => result.status === "fulfilled").length;

                retryResults.forEach((result) => {
                    if (result.status === "rejected") {
                        console.error("Failed to retry file parse:", result.reason);
                    }
                });
                ...
                failedFilesRetryFeedback.value = backend.translate(`${retriedCount} failed file(s) queued for retry`);
                if (failedFilesRetryFeedbackTimer !== null) {
                    window.clearTimeout(failedFilesRetryFeedbackTimer);
                }
                failedFilesRetryFeedbackTimer = window.setTimeout(() => {
                    failedFilesRetryFeedbackTimer = null;
                    failedFilesRetryFeedback.value = "";
                }, 2600);
            } finally {
                isRetryingFailedFiles.value = false;
            }
        };

        const handleClearAllFiles = async () => {
            const filesToRemove = uploadFilesStore.uploadedFiles.map((file) => file.filePath);
            if (
            ...
            try {
                ...
                removalResults.forEach((result) => {
                    if (result.status === "rejected") {
                        console.error("Failed to remove attached file:", result.reason);
                    }
                });
                ...
                attachmentsBulkFeedback.value = backend.translate(`${removedCount} file(s) removed`);
                if (attachmentsBulkFeedbackTimer !== null) {
                    window.clearTimeout(attachmentsBulkFeedbackTimer);
                }
                attachmentsBulkFeedbackTimer = window.setTimeout(() => {
                    attachmentsBulkFeedbackTimer = null;
                    attachmentsBulkFeedback.value = "";
                }, 2600);
            } finally {
                isClearingAllFiles.value = false;
            }
        };
        ...
        return {
            ...
            fileStatusSummary,
            fileGuidanceTitle,
            fileGuidanceDescription,
            filePromptSuggestions,
            failedUploadedFiles,
            isClearingAllFiles,
            isRetryingFailedFiles,
            isRemovingFailedFiles,
            failedFilesCleanupFeedback,
            failedFilesRetryFeedback,
            attachmentsBulkFeedback,
            applySuggestedPrompt,
            handleRemoveFailedFiles,
            ...
            >
                ...
                {hasUploadedFiles && (
                    <div class="input-area__attachments">
                        <div class="input-area__attachments-stack">
                            <div class="input-area__file-guidance chat-view__jump-target--files">
                                <div class="input-area__file-guidance-copy">
                                    <div class="input-area__file-guidance-title">{this.fileGuidanceTitle}</div>
                                    <div class="input-area__file-guidance-desc">{this.fileGuidanceDescription}</div>
                                </div>
                                <div class="input-area__file-guidance-side">
                                    <div class="input-area__file-guidance-stats">
                                        <span class="input-area__file-guidance-stat">
                                            {this.fileStatusSummary.total}{" "}
                                            {this.fileStatusSummary.total === 1 ? "file" : "files"}
                                        </span>
                                        {this.fileStatusSummary.parsingCount > 0 && (
                                            <span class="input-area__file-guidance-stat">
                                                {this.fileStatusSummary.parsingCount} parsing
                                            </span>
                                        )}
                                        {this.fileStatusSummary.errorCount > 0 && (
                                            <span class="input-area__file-guidance-stat input-area__file-guidance-stat--warning">
                                                {this.fileStatusSummary.errorCount} failed
                                            </span>
                                        )}
                                    </div>
                                    <div class="input-area__file-guidance-actions">
                                        <button
                                            type="button"
                                            class="input-area__file-guidance-action"
                                            data-attachment-action="clear-all"
                                            disabled={
                                                this.$props.disabled ||
                                                this.isClearingAllFiles ||
                                                this.isRetryingFailedFiles ||
                                                this.isRemovingFailedFiles
                                            }
                                            onClick={this.handleClearAllFiles}
                                        >
                                            {this.isClearingAllFiles ? "Removing all files..." : "Clear all"}
                                        </button>
                                    </div>
                                </div>
                            </div>

                            {this.failedUploadedFiles.length > 0 && (
                                <div class="input-area__failed-files-row chat-view__jump-target--failed-files">
                                    <div class="input-area__failed-files-copy">
                                        <span class="input-area__failed-files-label">Failed files:</span>
                                        <span class="input-area__failed-files-list">
                                            {this.failedUploadedFiles.map((file) => file.fileName).join(", ")}
                                        </span>
                                    </div>
                                    <button
                                        type="button"
                                        class="input-area__failed-files-action"
                                        data-attachment-action="retry-failed-files"
                                        disabled={
                                            this.$props.disabled ||
                                            this.isClearingAllFiles ||
                                            this.isRetryingFailedFiles ||
                                    ...
                                    >
                                        {this.isRetryingFailedFiles ? "Retrying failed files..." : "Retry failed files"}
                                    </button>
                                    <button
                                        type="button"
                                        class="input-area__failed-files-action"
                                        data-attachment-action="remove-failed-files"
                                        disabled={
                                            this.$props.disabled ||
                                            this.isClearingAllFiles ||
                                            this.isRetryingFailedFiles ||
                                        ...
                                        onClick={this.handleRemoveFailedFiles}
                                    >
                                        {this.isRemovingFailedFiles ? "Removing failed files..." : "Remove failed files"}
                                    </button>
                                </div>
                            )}
                            {this.failedFilesRetryFeedback && (
                                <div class="input-area__failed-files-feedback input-area__failed-files-feedback--retry">
                                    {this.failedFilesRetryFeedback}
                                </div>
                            )}
                            {this.attachmentsBulkFeedback && (
                                <div class="input-area__failed-files-feedback input-area__failed-files-feedback--bulk">
                                    {this.attachmentsBulkFeedback}
                                </div>
                            )}
                            {this.failedFilesCleanupFeedback && (
                                <div class="input-area__failed-files-feedback">
                                    {this.failedFilesCleanupFeedback}
                                </div>
                            )}

                            <div class="input-area__file-prompts">
                                {this.filePromptSuggestions.map((prompt) => (
                                    <div key={prompt} class="input-area__file-prompt-chip-group">
                                        <button
                                            type="button"
                                            class="input-area__file-prompt-chip"
                                            disabled={this.$props.disabled}
                                            onClick={() => this.applySuggestedPrompt(prompt, true)}
                                        >
                                            {prompt}
                                        </button>
                                        <button
                                            type="button"
                                            class="input-area__file-prompt-fill"
                                            disabled={this.$props.disabled}
                                            title="Fill only"
                                            onClick={() => this.applySuggestedPrompt(prompt, false)}
                                        >
                                            +
                                        </button>
                                    </div>
