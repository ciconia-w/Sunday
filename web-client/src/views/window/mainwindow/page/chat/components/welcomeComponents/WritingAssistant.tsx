...
function useDynamicCardWidth(containerRef: Ref<HTMLElement | undefined>) {
    ...
    onMounted(() => {
        setTimeout(() => {
            calculateLayout();
            if (containerRef.value && !disableResizeObserversForQt) {
...
export default defineComponent({
    ...
    setup() {
        ...
        const maybeAutoOpenRecentDoc = async () => {
            const url = new URL(window.location.href);
            const autoOpenRecentDoc = url.searchParams.get("autoOpenRecentDoc");
            if (!autoOpenRecentDoc || autoOpenRecentDoc === "0" || autoOpenRecentDoc === "false") {
                return;
            }

            for (let attempt = 0; attempt < 30; attempt += 1) {
                if (docs.value.length > 0) {
                    document.body.setAttribute("data-writing-assistant-auto-open", "ready");
                    await handleDocClick(docs.value[0] as RecentDoc);
                    document.body.setAttribute("data-writing-assistant-auto-opened-doc", docs.value[0]?.id || "");
                    return;
                }
                await new Promise((resolve) => setTimeout(resolve, 250));
            }

            document.body.setAttribute("data-writing-assistant-auto-open", "timeout");
        };

        onMounted(() => {
            void maybeAutoOpenRecentDoc();
        });
        ...
        const handleDocClick = async (doc: RecentDoc) => {
            ...
            console.log("[WritingAssistant] Opening recent doc:", doc.id, doc.conversation_id);
            document.body.setAttribute("data-writing-assistant-open-doc-request", doc.id);
            ...
            if (!article) {
                ...
                document.body.setAttribute("data-writing-assistant-open-doc-status", "failed");
                return;
            }

            document.body.setAttribute("data-writing-assistant-open-doc-status", "loaded");
            document.body.setAttribute("data-writing-assistant-open-doc-title", article.title || "");
            ...
            document.body.setAttribute("data-writing-assistant-open-doc-status", "opened");
        };
