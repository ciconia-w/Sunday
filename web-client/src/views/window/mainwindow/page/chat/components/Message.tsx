...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const mapUploadFilesToDisplayFiles = (files: UploadFile[]): DisplayFile[] => {
        ...
        const summarizeDisplayFiles = (files: DisplayFile[]) => {
            const total = files.length;
            const parsingCount = files.filter(
                (file) => file.parseStatus === "pending" || file.parseStatus === "parsing",
            ).length;
            ...
            return { total, parsingCount, errorCount, readyCount };
        };
        ...
        const cachedDisplayFiles = computed<DisplayFile[]>(() => {
            return mapUploadFilesToDisplayFiles(cachedFiles.value);
        });

        const cachedFileSummary = computed(() => {
            const files = cachedDisplayFiles.value;
            const { total, parsingCount, errorCount, readyCount } = summarizeDisplayFiles(files);
            const failedFileNames = files
                .filter((file) => file.parseStatus === "error")
                .map((file) => file.fileNameText)
                .filter(Boolean);
            ...
            if (parsingCount > 0) {
                title = backendStore.translate("Files attached");
                detail = backendStore.translate(
                    `${parsingCount} file(s) were still parsing when this message was sent.`,
                );
            } else if (errorCount > 0) {
                title = backendStore.translate("Files attached with warnings");
                detail = backendStore.translate(
                    `${readyCount} ready, ${errorCount} failed to parse when this message was sent.`,
                );
            } else if (total > 0) {
                title = backendStore.translate("Files attached");
                detail = backendStore.translate(
                    `${total} file(s) were attached as context for this request.`,
                );
            }
        ...
        const previousMessageFiles = computed<UploadFile[]>(() => {
            const uploadedFiles = props.previousMessage?.extension?.uploadedFiles;
            if (!Array.isArray(uploadedFiles)) {
                return [];
            }
        ...
        const messagePromptSource = computed(() => {
            const promptSource = props.message.extension?.promptSource;
            return typeof promptSource === "string" ? promptSource : "";
        });

        const previousMessagePromptSource = computed(() => {
            const promptSource = props.previousMessage?.extension?.promptSource;
            return typeof promptSource === "string" ? promptSource : "";
        });

        const previousMessageDisplayFiles = computed<DisplayFile[]>(() =>
            mapUploadFilesToDisplayFiles(previousMessageFiles.value),
        );

        const previousMessageFileSummary = computed(() => {
            const files = previousMessageDisplayFiles.value;
            const { total, parsingCount, errorCount, readyCount } = summarizeDisplayFiles(files);
            ...
            let title = backendStore.translate("Working from attached files");
            let detail = backendStore.translate(`${total} file(s) were attached to the request above.`);

            if (parsingCount > 0) {
                detail = backendStore.translate(
                    `${parsingCount} file(s) were still parsing when the assistant started this reply.`,
                );
            } else if (errorCount > 0) {
                title = backendStore.translate("Working from attached files with warnings");
                detail = backendStore.translate(
                    `${readyCount} ready, ${errorCount} failed to parse for the request above.`,
                );
            }
        ...
        const handleFollowUpClick = async (event: MouseEvent) => {
            event.stopPropagation();

            const previousUserMessage = props.previousMessage;
            if (!previousUserMessage) {
                return;
            }

            const previousTextContent = extractTextContent(previousUserMessage);
            const previousUploadedFiles = previousMessageFiles.value;

            await createDefaultConversation();
            uploadFilesStore.clearFiles();
            if (previousUploadedFiles.length > 0) {
                const filesByCategory = new Map<string | undefined, string[]>();
                previousUploadedFiles.forEach((file) => {
                    const category = file.category as unknown as DocFileCategory | undefined;
                    const categoryKey = category === undefined ? undefined : String(category);
                    if (!filesByCategory.has(categoryKey)) {
                        filesByCategory.set(categoryKey, []);
            ...
            if (chatInputContext) {
                chatInputContext.fillInput(previousTextContent, "replace");
                chatInputContext.focusInput();
            }
        };
            ...
            if (msg.role === UserType.USER) {
                ...
                const actionButtons: Array<{
                    ...
                    },
                    {
                        ...
                        onClick: (event: MouseEvent) => void handleFollowUpClick(event),
                        tooltip: backendStore.translate("Open branch"),
                        actionId: "open-branch",
                    },
                ];
        ...
        return {
            ...
            previousMessageFileSummary,
            messagePromptSource,
            previousMessagePromptSource,
            ...
            <div class="message__item" onMouseleave={() => this.handleMessageMouseLeave()}>
                <div class={`message__content ${msg.role === UserType.USER ? "message__content--user" : ""}`}>
                    ...
                    >
                        ...
                        >
                            {msg.role === UserType.USER && this.cachedDisplayFiles.length > 0 && (
                                <div class="message__attachment-context">
                                    <div class="message__attachment-header">
                                        <div class="message__attachment-copy">
                                            <div class="message__attachment-title">{this.cachedFileSummary.title}</div>
                                            <div class="message__attachment-detail">{this.cachedFileSummary.detail}</div>
                                        </div>
                                        <div class="message__attachment-stats">
                                            <span class="message__attachment-stat">
                                                {this.cachedFileSummary.total}{" "}
                                                {this.cachedFileSummary.total === 1 ? "file" : "files"}
                                            </span>
                                            {this.cachedFileSummary.parsingCount > 0 && (
                                                <span class="message__attachment-stat">
                                                    {this.cachedFileSummary.parsingCount} parsing
                                                </span>
                                            )}
                                            {this.cachedFileSummary.errorCount > 0 && (
                                                <span class="message__attachment-stat message__attachment-stat--warning">
                                                    {this.cachedFileSummary.errorCount} failed
                                                </span>
                                            )}
                                        </div>
                                    </div>
                                    {this.cachedFileSummary.failedFileNames.length > 0 && (
                                        <div class="message__attachment-warning-list">
                                            <span class="message__attachment-warning-label">
                                                {backendStore.translate("Failed files")}:
                                            </span>
                                            <span class="message__attachment-warning-names">
                                                {this.cachedFileSummary.failedFileNames.join(", ")}
                                            </span>
                                        </div>
                                    )}
                                    <FileGroup
                            ...
                            {msg.role === UserType.USER && this.messagePromptSource === "file-guidance" && (
                                <div class="message__prompt-source-badge chat-view__jump-target--source">
                                    {backendStore.translate("Sent from file suggestion")}
                                </div>
                            )}
                            {msg.role === UserType.ASSISTANT && this.previousMessageFileSummary && (
                                <div class="message__reply-context">
                                    <div class="message__reply-context-title">
                                        {this.previousMessageFileSummary.title}
                                    </div>
                                    <div class="message__reply-context-detail">
                                        {this.previousMessageFileSummary.detail}
                                    </div>
                                    {this.previousMessagePromptSource === "file-guidance" && (
                                        <div class="message__reply-context-source chat-view__jump-target--source">
                                            {backendStore.translate("Answering a file-guided request")}
                                        </div>
                                    )}
                                </div>
                            )}
