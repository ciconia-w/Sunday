...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const disableResizeObserversForQt =
            typeof window !== "undefined" && window.location.search.includes("disableResizeObservers=1");
        ...
        onMounted(() => {
            console.log("[MarkdownEditor] mounted", props.articleId, props.conversationId);
            document.body.setAttribute("data-markdown-editor-mounted", "true");
            document.body.setAttribute("data-markdown-editor-article-id", props.articleId || "");
            document.body.setAttribute("data-markdown-editor-conversation-id", props.conversationId || "");
            if (layoutEl.value && !disableResizeObserversForQt) {
        ...
        const autoEditApplied = ref(false);
        ...
        const maybeAutoEditForSmoke = async (source: string) => {
            if (autoEditApplied.value) {
                return;
            }
            const url = new URL(window.location.href);
            const autoAppend = url.searchParams.get("autoAppendToArticle");
            if (!autoAppend) {
                return;
            }

            const expectedArticleId = url.searchParams.get("autoAppendArticleId");
            if (expectedArticleId && expectedArticleId !== props.articleId) {
                return;
            }

            await new Promise((resolve) => setTimeout(resolve, 300));
            const currentContent = currentEditorContent.value || "";
            if (currentContent.includes(autoAppend)) {
                autoEditApplied.value = true;
                return;
            }

            const nextContent = `${currentContent}${autoAppend}`;
            autoEditApplied.value = true;

            try {
                if (editorInstance.value) {
                    editorInstance.value.action(replaceAll(normalizeMathDelimiters(nextContent)));
                }
            } catch (error) {
                console.warn("[MarkdownEditor] auto-edit replaceAll failed", error);
            }

            handleContentUpdate(nextContent);
            document.body.setAttribute("data-markdown-editor-auto-edit", "applied");
            document.body.setAttribute("data-markdown-editor-auto-edit-article-id", props.articleId || "");
            console.log("[MarkdownEditor] auto-edit applied", props.articleId, source);
        };
        ...
        onBeforeUnmount(() => {
            document.body.removeAttribute("data-markdown-editor-mounted");
            document.body.removeAttribute("data-markdown-editor-article-id");
            document.body.removeAttribute("data-markdown-editor-conversation-id");
            document.body.removeAttribute("data-markdown-editor-auto-edit");
            document.body.removeAttribute("data-markdown-editor-auto-edit-article-id");
            // 清理宽度检测 observer
        ...
        const handleEditorReady = (editor: any) => {
            ...
            console.log("[MarkdownEditor] editor ready", props.articleId, props.conversationId);
            void maybeAutoEditForSmoke("editor-ready");
        ...
        onMounted(() => {
            void maybeAutoEditForSmoke("mounted");
        });
