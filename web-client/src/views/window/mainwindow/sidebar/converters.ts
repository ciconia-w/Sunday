...
import type { ConversationIndexWithStatus } from "@/types/conversation";
import {
...
export function convertAssistantListToSidebarGroup(
    ...
    const rawAssistantItems: SidebarItem[] = [];
...
export function createWorkspaceSidebarGroup(currentWorkspacePage?: string | null): SidebarGroup {
    const translate = useBackendStore().translate;

    const workspaceItems: SidebarItem[] = [
        {
            id: MAIN_WINDOW_WORKSPACE_PAGES.EXTENSIONS,
            type: "workspace",
            icon: "icon_extensions_v2",
            name: translate("扩展"),
            selected: currentWorkspacePage === MAIN_WINDOW_WORKSPACE_PAGES.EXTENSIONS,
            data: { workspacePage: MAIN_WINDOW_WORKSPACE_PAGES.EXTENSIONS },
        },
        {
            id: "automation",
            type: "workspace",
            icon: "super",
            name: translate("自动化"),
            selected: currentWorkspacePage === "automation",
            data: { workspacePage: "automation" },
        },
    ];
...
export function convertToSidebarGroups(sources: {
    ...
    currentWorkspacePage?: string | null;
    conversationIndexes?: ConversationIndexWithStatus[];
    temporaryVisibleAssistantId?: string | null;
    selectedAssistantId?: string | null;
    shouldHighlightChatContext?: boolean;
    /** 分组折叠状态映射 */
    ...
    groups.push(createWorkspaceSidebarGroup(sources.currentWorkspacePage));
