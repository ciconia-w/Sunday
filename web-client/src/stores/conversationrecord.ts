...
type ToolLikeData = Pick<ToolUseData, "name" | "status" | "index" | "params" | "result" | "details" | "display_content">;
...
export class ConversationRecord {
    ...
    handleSessionMessage(sessionId: string, message: string) {
        try {
            ...
            // parsedMessage debug log removed
    ...
    normalizeToolUseData(rawData: unknown): ToolUseData | null {
        ...
        if (toolData.details && !this.isPlainObject(toolData.details)) {
            delete toolData.details;
        } else if (toolData.details) {
            toolData.details = { ...toolData.details };
        }
    ...
    mergeToolLikeData(target: ToolLikeData, incoming: ToolLikeData) {
        ...
        if (incoming.details !== undefined) target.details = incoming.details;
    ...
    updateAnsweringMessage(content: string) {
        if (!performance.getEntriesByName("sunday-first-text").length && content) { performance.mark("sunday-first-text"); }
        const lastMessage = this.getMessage(this._messageId); // 找到当前待填充消息
        if (!lastMessage) return;
