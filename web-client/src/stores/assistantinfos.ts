...
const RETAINED_ASSISTANTS: Assistant[] = [
    {
        id: AssistantID.UOS_AI_WRITING,
        name: "Writing",
...
export const useAssistantInfosStore = defineStore("assistantInfos", {
    state: () => ({
        ...
        assistantInventory: ref<Assistant[]>([]),
        loading: ref(true),
    ...
    getters: {
        ...
        // 获取完整助手清单（包含默认不暴露在主路径中的 retained assistants）
        getAssistantInventory: (state) => state.assistantInventory,
        // 获取助手区侧边栏顺序（包含固定入口项）
    ...
    actions: {
        ...
        setAssistantInventory(assistants: Assistant[]) {
            this.assistantInventory = assistants;
        },
        ...
        getAssistantById(id: string) {
            return (
                this.assistantInventory.find((assistant) => assistant.id === id) ??
                this.assistantList.find((assistant) => assistant.id === id)
            );
        },
        ...
        async loadAssistantList(
            ...
            try {
                ...
                const retainedAssistants = RETAINED_ASSISTANTS.filter(
                    (assistant) => !assistants.some((item: Assistant) => item.id === assistant.id),
                );
                this.setAssistantInventory([...assistants, ...retainedAssistants]);
                const visibleAssistants = assistants.filter((assistant: Assistant) => assistant.id === AssistantID.UOS_AI);
                ...
                const newAssistantIds = shouldExposeNewAssistants
                    ? getNewSidebarAssistantIds(previousAssistants, visibleAssistants)
                    : [];
                ...
                this.setAssistantList(sortAssistantsByOrder(visibleAssistants, nextOrder));
        ...
        async checkEnvironment() {
            ...
            if (assistant.id === AssistantID.UOS_AI_KNOWLEDGE_BASE) {
                ...
                if (!isKnowledgeBaseReady) {
                    ...
                    message = backend.translate(
                        "This retained extension flow requires the embedding plugin to be installed first.",
                    );
            ...
            if (assistant.id === AssistantID.UOS_AI_MCP_AND_SKILL) {
                ...
                if (!isMcpRuntimeReady) {
                    ...
                    isShowToast = true;
                    message = backend.translate(
                        "This retained extension flow requires the Sunday runtime dependency to be installed first.",
                    );
