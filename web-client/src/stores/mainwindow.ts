...
const EXTENSIONS_WORKSPACE_TABS = new Set<MainWindowWorkspacePage>([
...
export const useMainWindowStore = defineStore("mainwindow", {
    state: () => ({
        ...
        extensionsActiveTab: MAIN_WINDOW_WORKSPACE_PAGES.SKILLS as MainWindowWorkspacePage,
        sidebarWidth: 200,
    ...
    actions: {
        normalizeWorkspacePageTarget(page: MainWindowWorkspacePage) {
            if (EXTENSIONS_WORKSPACE_TABS.has(page)) {
                return {
                    workspacePage: MAIN_WINDOW_WORKSPACE_PAGES.EXTENSIONS as MainWindowWorkspacePage,
                    extensionsActiveTab: page,
                };
            }
            ...
            return {
                workspacePage: page,
                extensionsActiveTab:
                    page === MAIN_WINDOW_WORKSPACE_PAGES.EXTENSIONS ? this.extensionsActiveTab : null,
            };
        },
        ...
        async transitionWorkspacePage(
            ...
            const normalizedTarget = this.normalizeWorkspacePageTarget(page);
            const targetPage = normalizedTarget.workspacePage;
            const currentPage = this.workspacePage;

            if (normalizedTarget.extensionsActiveTab) {
                this.extensionsActiveTab = normalizedTarget.extensionsActiveTab;
            }

            if (targetPage === currentPage) {
                return this.runWorkspacePageEnter(targetPage, currentPage);
            }

            await this.runWorkspacePageLeave(currentPage, targetPage);

            if (!options?.skipNavigationStackUpdate) {
                const targetPageDefinition = getMainWindowWorkspacePage(targetPage);
        ...
        openMcpServicesPage() {
            return this.openExtensionsPage(MAIN_WINDOW_WORKSPACE_PAGES.MCP_SERVICES);
        },

        openSettingsHomePage() {
            return this.openWorkspacePage(MAIN_WINDOW_WORKSPACE_PAGES.SETTINGS_HOME);
        },

        openModelSettingsPage() {
            return this.openWorkspacePage(MAIN_WINDOW_WORKSPACE_PAGES.MODEL_SETTINGS);
        },

        openSkillsPage() {
            return this.openExtensionsPage(MAIN_WINDOW_WORKSPACE_PAGES.SKILLS);
        },
