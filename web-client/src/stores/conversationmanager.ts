...
export const useConversationManagerStore = defineStore("conversationmanager", {
    state: () => ({
        ...
        iCompNotificationTargets: {} as Record<
            number,
            { conversationId: string; messageId: string; requestId: string }
        >, // IComp 通知ID -> 目标信息
        iCompNotificationListenerInitialized: false, // IComp 系统通知 action 监听是否已注册
    }),
    ...
    getters: {
        ...
        getCurrentMessagesRender: (state) => {
            const conversation = state.conversionList.get(state.currentConversationId)?.conversation;
            if (!conversation) {
                return null;
            }

            // 会话内部消息在流式阶段会被原地修改，返回新的外层引用可确保依赖视图重新计算。
            return {
    ...
    actions: {
        ...
        initializeICompNotificationHandlers() {
            if (this.iCompNotificationListenerInitialized) {
                return;
            }
            const notifyStore = useNotifyStore();
            notifyStore.onSystemNotificationAction(({ notificationId, actionKey }) => {
                void this.handleICompNotificationAction(notificationId, actionKey);
            });
            this.iCompNotificationListenerInitialized = true;
        },
        ...
        async showICompNotification(
            ...
            if (data.ic_type === "bash_approve") {
                actions.push(
                    { key: "view_now", text: backend.translate("View Now") },
                    { key: `bash_approved_${requestId}`, text: backend.translate("Allow Once") },
                    { key: `bash_always_approve_${requestId}`, text: backend.translate("Allow Chat") },
                    { key: `bash_reject_${requestId}`, text: backend.translate("Reject") },
                );
            } else if (data.ic_type === "file_change_approve") {
                actions.push(
                    { key: "view_now", text: backend.translate("View Now") },
                    { key: `bash_approved_${requestId}`, text: backend.translate("Apply") },
                    { key: `bash_reject_${requestId}`, text: backend.translate("Reject") },
                );
            }
            ...
            this.iCompNotificationTargets[notificationId] = {
                conversationId,
                messageId,
                requestId,
            };
        ...
        async handleICompNotificationAction(notificationId: number, actionKey: string) {
            console.log("[ICompNotification] Action:", notificationId, actionKey);
            const target = this.iCompNotificationTargets[notificationId];
            if (!target) {
                return;
            }

            delete this.iCompNotificationTargets[notificationId];
            const notifyStore = useNotifyStore();
            await notifyStore.closeSystemNotification(notificationId);

            const conversationRecord = this.conversionList.get(target.conversationId) as ConversationRecord | undefined;
            if (!conversationRecord) {
                return;
            }
            ...
            if (actionKey.startsWith(`bash_approved_${target.requestId}`)) {
                approved = true;
            } else if (actionKey.startsWith(`bash_always_approve_${target.requestId}`)) {
                approved = true;
                alwaysApprove = true;
            } else if (actionKey.startsWith(`bash_reject_${target.requestId}`)) {
                approved = false;
            } else {
                return;
            }
            ...
            conversationRecord.updateICompStatus(target.messageId, target.requestId, status);

            const message = conversationRecord.getMessage(target.messageId);
            const icItem = message?.render_message.find(
                (item) =>
                    item.type === "interactive_components" &&
                    (item.data as BashApproveData | FileChangeApproveData).id === target.requestId,
            );
            const icType = (icItem?.data as BashApproveData | FileChangeApproveData)?.ic_type ?? "bash_approve";
         ...
         */
        // 处理会话开始事件
        handleSessionStarted(sessionId: string, conversationId?: string) {
            const [isValid, conversationRecord] = this.validateSessionId(sessionId, conversationId); // 校验sessionId是否有效
        ...
        // 处理会话消息事件
        handleSessionMessage(sessionId: string, message: string, conversationId?: string) {
            const [isValid, conversationRecord] = this.validateSessionId(sessionId, conversationId); // 校验sessionId是否有效
            if (!isValid || !conversationRecord) {
                return; // 校验失败，直接返回
            }
        ...
        // 处理会话完成事件
        async handleSessionFinished(sessionId: string, message: string, hintedConversationId?: string) {
            const [isValid, conversationRecord] = this.validateSessionId(sessionId, hintedConversationId); // 校验sessionId是否有效
            if (!isValid || !conversationRecord) {
                return; // 校验失败，直接返回
            }
            const mappedConversationId = this.answeringSession.get(sessionId);
            await conversationRecord.handleSessionFinished(sessionId, message);
            if (mappedConversationId) {
                const isCurrentConversation = this.currentConversationId === mappedConversationId;
                // 当前正在查看的会话一旦完成回复，应立即转为已读。
                if (isCurrentConversation) {
                    conversationRecord.markAsRead(true);
                } else {
                    conversationRecord.markAsUnread();
                    this.scheduleAiReplyCompletedNotification(mappedConversationId, conversationRecord.messageId);
                }
            }
        ...
        // 处理会话错误事件
        async handleSessionError(sessionId: string, message: string, hintedConversationId?: string) {
            const [isValid, conversationRecord] = this.validateSessionId(sessionId, hintedConversationId); // 校验sessionId是否有效
            if (!isValid || !conversationRecord) {
                return; // 校验失败，直接返回
            }
        ...
        // 校验sessionId是否有效
        validateSessionId(sessionId: string, hintedConversationId?: string): [boolean, ConversationRecord | null] {
            if (hintedConversationId) {
                const hintedConversationRecord = this.conversionList.get(hintedConversationId) ?? null;
                if (hintedConversationRecord?.root?.id) {
                    hintedConversationRecord.sessionId = sessionId;
                    this.answeringSession.set(sessionId, hintedConversationRecord.root.id);
                    return [true, hintedConversationRecord];
                }
            }
            ...
            if (!conversationId) {
                const fallbackConversationRecord =
                    [...this.conversionList.values()].find((record) => record.sessionId === sessionId) ?? null;

                if (fallbackConversationRecord?.root?.id) {
                    this.answeringSession.set(sessionId, fallbackConversationRecord.root.id);
                    return [true, fallbackConversationRecord];
                }

                const currentConversationRecord = this.conversionList.get(this.currentConversationId) ?? null;
                if (
                    currentConversationRecord?.root?.id &&
                    Object.keys(currentConversationRecord.messages || {}).length > 0
                ) {
                    currentConversationRecord.sessionId = sessionId;
                    this.answeringSession.set(sessionId, currentConversationRecord.root.id);
                    return [true, currentConversationRecord];
                }

                const generatingRecords = [...this.conversionList.values()].filter(
                    (record) => record.conversationStatus === ConversationStatus.Generating && !!record.root?.id,
                );
                if (generatingRecords.length === 1) {
                    const [singleGeneratingRecord] = generatingRecords;
                    singleGeneratingRecord.sessionId = sessionId;
                    this.answeringSession.set(sessionId, singleGeneratingRecord.root!.id);
                    return [true, singleGeneratingRecord];
                }

                console.warn(
                    "validateSessionId fallback failed:",
                    JSON.stringify({
                        sessionId,
                        currentConversationId: this.currentConversationId,
                        currentConversationMessageCount:
                            Object.keys(currentConversationRecord?.messages || {}).length,
                        generatingConversationIds: generatingRecords
