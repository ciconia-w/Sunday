...
import { ZH_CN_TRANSLATIONS } from "@/i18n/zhCN";

const detectInitialChineseLanguage = () => {
    if (typeof navigator !== "undefined") {
        const locales = navigator.languages?.length ? navigator.languages : [navigator.language];
        return locales.some((locale) => typeof locale === "string" && locale.toLowerCase().startsWith("zh"));
    }
...
export const useBackendStore = defineStore("backend", {
    state: () => ({
        ...
        reportChannel: null,
        translations: {},
        isChineseLanguage: detectInitialChineseLanguage(), // 是否为中文环境
    ...
    actions: {
        ...
        translate(key: string) {
            ...
            if (this.isChineseLanguage) {
                return ZH_CN_TRANSLATIONS[key] || key;
            }
